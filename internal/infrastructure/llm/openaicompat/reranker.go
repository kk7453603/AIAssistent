...
// LLMReranker implements ports.Reranker using pointwise relevance scoring via LLM.
...
func NewReranker(client *Client) *LLMReranker {
	return &LLMReranker{client: client, concurrency: 5}
}
...
func (r *LLMReranker) Rerank(ctx context.Context, query string, chunks []domain.RetrievedChunk, topN int) ([]domain.RetrievedChunk, error) {
	...
	for _, res := range results {
		if res.err != nil {
			// On any LLM error, return chunks unchanged (fallback).
			return chunks, nil
		}
...
%s`, query, chunk.Filename, text)
	...
	respText, err := r.client.chatCompletion(ctx, []chatMessage{
	...
	return result.Score / 10.0, nil // normalize to 0..1
}
